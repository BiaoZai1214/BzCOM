...
// !!! 如果要使用软件IIC,就把Hard前缀换成Soft,解锁Wait4Ack()
void OLED_SendData(uint8_t *Data, uint16_t len)
{
    if (len == 0) return;
    
    HardI2C_Start();
    HardI2C_SendAddr(OLED_ADDRESS);
    //SoftI2C_Wait4Ack();  // 软件记得开这个接收Ack

    for (uint16_t i = 0; i < len; i++)
    {
        HardI2C_SendByte(Data[i]);   // 连续发多字节数据
        //SoftI2C_Wait4Ack(); 
    }
    HardI2C_Stop();
}
...
void OLED_ShowFrame() 
{
    ...
    for (uint8_t i = 0; i < OLED_PAGE; i++) {
      ...
      OLED_SendCmd(0x02);     // 设置列地址低4位(0x00 + 列地址偏移量2)
      OLED_SendCmd(0x10);     // 设置列地址高4位
      memcpy(sendBuffer + 1, OLED_GRAM[i], OLED_COLUMN);
      OLED_SendData(sendBuffer, OLED_COLUMN + 1);
    }
}

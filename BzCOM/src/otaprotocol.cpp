...
QByteArray OtaProtocol::buildFrame(quint16 cmd, const QByteArray &data)
{
    ...
    frame.append(static_cast<char>(Proto::Head1));
    frame.append(static_cast<char>(Proto::Head2));
...
{
    while (buffer.size() >= Proto::FixedLen + Proto::CrcLen) {
        int headPos = buffer.indexOf(Proto::Head1);
        if (headPos < 0) { buffer.clear(); break; }
        if (headPos + 1 >= buffer.size()) break;       // 0x55在末尾，等更多数据
        if (static_cast<quint8>(buffer[headPos + 1]) != Proto::Head2) {
...
{
    if (frame.size() < Proto::FixedLen + Proto::CrcLen) return false;
    quint16 rxCrc = static_cast<quint8>(frame[frame.size() - 2])
                  | (static_cast<quint8>(frame[frame.size() - 1]) << 8);
    quint16 dataLen = static_cast<quint8>(frame[3])
                    | (static_cast<quint8>(frame[4]) << 8);
    quint16 calcCrc = calcCRC16(
        reinterpret_cast<const quint8 *>(frame.constData() + 2), dataLen + 6);
...
{
    if (frame.size() < Proto::FixedLen) return QByteArray();
    quint16 dataLen = static_cast<quint8>(frame[3])
                    | (static_cast<quint8>(frame[4]) << 8);

...
void OTAController::resetSession(void)
{
    m_pktNum = 0;
    m_sentSize = 0;
    m_totalSize = 0;
...
bool OTAController::openFile(const QString &filePath)
{
    m_file.setFileName(filePath);
    if (!m_file.open(QIODevice::ReadOnly)) {
        emit otaFinished(false, "无法打开文件");
        return false;
    }
...
void OTAController::onDataReceived(const QByteArray &data)
{
    ...
    switch (m_state) {
        case STATE_START_WAIT:
            if (cmd == Cmd::ack(Cmd::UpdateStart)) { emit progress("✓ START ACK", 0); sendNextPacket(); }
            break;
        case STATE_DATA_WAIT:
            if (cmd == Cmd::ack(Cmd::UpdateData)) sendNextPacket();
            else if (cmd == Cmd::nak(Cmd::UpdateData)) { emit progress(QString("⚠ NAK，重发第%1包").arg(m_pktNum), 2); resendLastPacket(); }
            break;
        case STATE_END_WAIT:
            if (cmd == Cmd::ack(Cmd::UpdateEnd)) { emit otaFinished(true, QString("成功，共%1包").arg(m_pktNum)); stop(); }
            break;
        default: break;
    }
}

void OTAController::onTimeout(void)
{
    const char *msg;
    switch (m_state) {
...
void OTAController::sendStartFrame(void)
{
    QByteArray frame = OtaProtocol::instance().buildFrame(Cmd::UpdateStart, OtaProtocol::toBytes32(m_totalSize));
    emit sendData(frame);
...
void OTAController::sendNextPacket(void)
{
    ...
    QByteArray frame = OtaProtocol::instance().buildFrame(Cmd::UpdateData, payload);
    emit sendData(frame);
...
void OTAController::resendLastPacket(void)
{
...
void OTAController::sendEndFrame(void)
{
    QByteArray frame = OtaProtocol::instance().buildFrame(Cmd::UpdateEnd, OtaProtocol::toBytes32(m_totalSize));
    emit sendData(frame);
...
void OTAController::sendNoCheck(void)
{
    int pktNum = 0;
    while (!m_file.atEnd()) {

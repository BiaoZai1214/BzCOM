...
void MainWindow::initSignalSlots(void)
{
    ...
    // 每次按键重启延时定时器，停止输入80ms后才计算校验和
    connect(ui->textEdit, &QTextEdit::textChanged, this, [this]() { m_checksumTimer.start(); });
...
void MainWindow::updateChecksum(void)
{
    ...
    if (!input.isEmpty()) {
        QByteArray data;
        if (m_isHexMode) {
            data = CheckSumUtil::parseHex(input);
        } else {
            data = input.toUtf8();
        }
...
void MainWindow::sendText(void)
{
    ...
    QByteArray rawData;
    if (m_isHexMode) {
        rawData = CheckSumUtil::parseHex(input);
    ...
    if (csType > 0) {
        QByteArray cs = CheckSumUtil::calculate(rawData, csType);
...
void MainWindow::setOtaBusy(bool busy)
{
    ...
    // OTA期间断开原始数据→日志，避免二进制协议帧显示为乱码
    static bool s_dataDisconnected = false;
    if (busy && !s_dataDisconnected) {
        for (int i = 0; i < 3; i++)
            disconnect(m_widgets[i], &ComWidget::dataReceived, m_log, &LogPanel::appendData);
        s_dataDisconnected = true;
    } else if (!busy && s_dataDisconnected) {
        for (int i = 0; i < 3; i++)
            connect(m_widgets[i], &ComWidget::dataReceived, m_log, &LogPanel::appendData);
        s_dataDisconnected = false;
    }
}
...
{
    m_checksumTimer.setSingleShot(true);
    m_checksumTimer.setInterval(80);
    connect(&m_checksumTimer, &QTimer::timeout, this, &MainWindow::updateChecksum);
